...
from superhero.schemas import HeroBaseSchema, HeroParamsSchema, HeroAddSchema
...
@router.post(
    path="/", summary="Добавление нового героя", response_model=HeroBaseSchema
)
async def api_add_hero(
    ...
    try:
        hero: HeroBaseSchema = await get_info_hero_service.execute(
            name=new_hero.name
        )
    except HeroInfoDontExistError:
        raise HTTPException(
            status_code=404,
            detail=f"Героя с именем {new_hero.name} не существует."
        )
